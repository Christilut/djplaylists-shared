...
const getPercentageColor = (value: number): { bg: string; text: string } => {
  ...
  else if (value >= 70) return { bg: 'bg-green-300/60', text: 'text-white' }
  else return { bg: 'bg-yellow-300/60', text: 'text-white' }
};
...
const PercentageCell = ({ value, className }: { value: number | undefined, className?: string }) => {
  ...
  return (
    <div className={cn("text-center", className)}>
      <div className={cn("inline-block px-2 py-1 rounded-md", bg, text)}>
        {percentage}%
      </div>
    </div>
  );
